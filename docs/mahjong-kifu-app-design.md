...
- **[決定]（2026-07）麻雀何切る問題機能**（作成・回答・一覧・マイ管理・共有。Plan: [docs/plans/nanikiru.md](plans/nanikiru.md)）。
  ...
  - **[決定]（2026-07 変更）正解（作者の答え）は設けない。** 多様な正解を前提に、回答者は自由に
    ...
    出題者のコメント（explanation・任意）を見る。
  - **[決定]（2026-07）相手の手牌は出題オプション（既定 OFF）。** 配牌何切る等、相手の手が重要でない
    問題では設定しない。設定する席には自分と同じ整合（13枚・読めない牌なし）を要求し、
    設定された手牌は回答者に表向きで見える（盤面は共有卓がそのまま描画）。
