...
### [決定] プロンプト方針
- 河は**1方向ごとの単方向プロンプト**（`river_reader_prompt.md` を1方向版へ修正して使う）。出力は `AiRiverResponse` 形式。
- 全牌に **confidence(0.0–1.0)** を出させる。読めない牌は `tile: null`＋スロット保持（枚数・順序を壊さない）。推測で埋めさせない。
- 萬子は「まず萬子と判定 → 数字を別途読む」と手順を分けさせる（誤読の定番対策）。
- **[未確定] JSON強制とtool併用の挙動**: Code Execution使用時、レスポンスが「テキスト＋生成コード＋実行結果」の混在パーツで返る。**位置でなく種類でパーツを仕分け**、最終テキストからJSONを拾うパース処理が必要。
