...
- **[決定]（2026-07-25）トレーニングクイズ機能（特訓）**（Plan: [docs/plans/quiz-training.md](plans/quiz-training.md)）。
  - 3種: **清一色多面待ち**（単色テンパイ13枚→待ち牌を全選択・完全一致のみ正解）／
    ...
    **点数計算**（[決定] 2026-07-26 追加・同日 v2。和了の牌姿（鳴き=ポン/チー/カン0〜3組・
    赤5・ドラ表示牌1〜2枚）＋条件（対局表記「東◯局 ◯家 (リーチ) ツモ/ロン」
    [決定] 2026-08-04 に旧「親（東家）・…・場風 東」から変更。親子は局と自風から読む・自風・
    **リーチ有無**（門前手の約5割。立直1翻・
  ...
  - **無料は1日10回・種目共通（セッション開始時に消費・JST 0時回復・サーバ強制）・有料は無制限**。
    Gemini 解析枠とは無関係の独立した制限。
    （[決定] 2026-08-04 オーナー: 3→10 に拡大。同日決定で**未サインインの匿名プレイも可**
    （API 非呼び出し・結果は保存しない）。Plan: [plans/quiz-open-and-ranking.md](plans/quiz-open-and-ranking.md)）
  - 履歴（kind・出題数・正解数・所要ms）を保存し、マイページに期間グラフ（成長の可視化）。
    **成績は本人のみ閲覧**（公開プロフィールには出さない。[決定] 2026-08-04 で**ランキング集計値の
    範囲のみ転換**: 検証済みセッションの集計を種目別ランキングとして公開する。詳細は
    [plans/quiz-open-and-ranking.md](plans/quiz-open-and-ranking.md) Phase 4）。
    計測イベントに成績・PII は送らない。
