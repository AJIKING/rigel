...
7. **画像は恒久保存し、所有者だけに見せ、データ削除で必ず消す。**（**[決定] 2026-08-03 変更**。旧「恒久保存しない」→ 元写真の検証・記録・再解析価値を採ってオーナー判断で転換。[docs/plans/photo-retention.md](docs/plans/photo-retention.md)）
   解析に使った撮影画像はバケット `rigel` に**送信時から恒久保存**し、半荘/何切るに紐づける。**閲覧は所有者のみ**（公開半荘・公開問題でも写真は露出しない・API は他人に 404）。**削除はデータ削除時のみ**（半荘削除・問題/下書き削除・退会で R2 も必ず掃除。局削除では消さない）。画像を他の用途（AI 学習・公開など）に使う口を増やさない。**公開範囲（public/private・既定 private）・ルール・選手情報（players）は半荘単位**（局ごとに持たず、変更は配下の全局へ一括反映・新局は引き継ぎ・局単位の PUT では書き換え不可）。private は所有者のみ閲覧可。
...
> **リリース準備（ストア提出）の残作業は [docs/plans/release.md](docs/plans/release.md) が
> マスターチェックリスト**（2026-08-05 起案。コード側 A / オーナー作業 B / 実機 C / 要判断 D）。
...
└── apps/
    ...
    ├── web/     web               # Next.js (App Router)。/kifu(公開牌譜一覧) /problems(公開何切る一覧) /training(特訓・匿名可) /ranking(特訓ランキング・公開) /mypage(マイページ=牌譜/何切る/お気に入り/特訓タブ) /mypage/training/[sessionId](特訓セッション詳細) /kifu/[gameId](0局=半荘ヘッダビュー・局あり=先頭局へ) /kifu/[gameId]/[logId](盤面エディタ) /k/[gameId](公開ビューア・動的OGP=lib/og-meta+opengraph-image) /p/[id](何切る回答) /problems/new・/problems/[id]/edit(何切る編集) /settings /u/[handle] /login
    └── mobile/  mobile            # Expo + react-navigation。タブ=牌譜/何切る/特訓/マイページ/設定 + Capture/GameDetail/Board/PublicGame/PublicUser/ProblemAnswer/ProblemEdit/TrainingSession/Ranking/Login
```
