...
6. Captured source images are persisted **permanently** (decided 2026-08-03, owner —
   reverses the previous no-persistence rule) in the `rigel` R2 bucket, uploaded at submit
   time and linked to the game / problem draft they belong to. Constraints: images are
   visible to the **owner only** (never exposed on public games/problems; 404 for others),
   they are deleted **only when their data is deleted** (game deletion, problem/draft
   deletion, account deletion — R2 must be cleaned alongside D1), and they must not be used
   for any other purpose (no AI training, no public serving).
   See docs/plans/photo-retention.md and CLAUDE.md rule 7.
